...
func benchmarkOTELSpanLimits(b *testing.B, limits sdktrace.SpanLimits) {
...
func BenchmarkSpanLimits(b *testing.B) {
	b.Run("AttributeValueLengthLimit", func(b *testing.B) {
		...
		benchmarkOTELSpanLimits(b, limits)
	})
...
func BenchmarkE2E(b *testing.B) {
	...
	tp := sdktrace.NewTracerProvider(
		sdktrace.WithSampler(sdktrace.AlwaysSample()), sdktrace.WithSpanProcessor(sdktrace.NewBatchSpanProcessor(exporter)))

	// Setup a full OTEL trace

	// Run the imaginary application which uses OTEL
